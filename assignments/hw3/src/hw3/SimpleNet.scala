...
class SimpleNet(val weights: Array[Matrix]) extends NNModule {
  ...
  protected val tail: NNModule = new LN(weights(10))

  def apply(input: NDArray): NDArray = {
    val batch = input.getShape(0)
    val isBatched = input.ndim >= 3
    val flat = if (isBatched) input.reshape(input.getShape(0), 784, 1) else input.reshape(784, 1)
    ...
    val x5 = tail(x4)
    val x6 = if (isBatched) x5.reshape(batch, 10) else x5.reshape(10)
    Softmax(x6)
  }
}
...
class SystolicNet(weights: Array[Matrix]) extends SimpleNet(weights) {
  ...
  override protected val tail: NNModule = new LN(weights(10))
}
